		...
		surface_domain *Surf, int verbose_level)
{
	...
	Poly2_x123 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	Poly3_x123 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	...
	Poly1_4 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	Poly2_4 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	...
	Partials = NEW_OBJECTS(algebra::ring_theory::partial_derivative, 4);
		...
		int verbose_level)
{
	...
	Poly2_27 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	Poly4_27 = NEW_OBJECT(algebra::ring_theory::homogeneous_polynomial_domain);
	...
	for (i = 0; i < nb_monomials2; i++) {
		...
		for (j = 0; j < nb_monomials2; j++) {
			...
			for (k = 0; k < nb_monomials2; k++) {
				...
				d = Surf->F->mult3(a, b, c);
				other::orbiter_kernel_system::Orbiter->Int_vec->add3(
	...
	int scalar, int *Ad, int verbose_level)
{
	...
	for (i = 0; i < nb_monomials2; i++) {
		...
		for (j = 0; j < nb_monomials2; j++) {
			...
			d = Surf->F->mult(a, b);
			other::orbiter_kernel_system::Orbiter->Int_vec->add(Poly2_27->get_monomial_pointer(i),
				Poly2_27->get_monomial_pointer(j),
				M, 27);
		...
		int *MM, int verbose_level)
{
	...
	for (i = 0; i < nb_monomials4; i++) {
		...
		for (j = 0; j < nb_monomials2; j++) {
			...
			d = Surf->F->mult(a, b);
			other::orbiter_kernel_system::Orbiter->Int_vec->add(Poly4_27->get_monomial_pointer(i),
				Poly2_27->get_monomial_pointer(j),
				M, 27);
			idx = Poly6_27->index_of_monomial(M);
			if (idx >= nb_monomials6) {
