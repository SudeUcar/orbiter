		...
		int verbose_level)
{
	...
	if (Sch1->Forest->nb_orbits == 1 && Sch2->Forest->nb_orbits == 1) {
		ost << "The group is transitive on points and on lines.\\\\" << endl;
		std::vector<int> Orb1;
		std::vector<int> Orb2;
		Sch1->Forest->get_orbit_in_order(Orb1, 0 /* orbit_idx */, verbose_level);
		Sch2->Forest->get_orbit_in_order(Orb2, 0 /* orbit_idx */, verbose_level);
	...
	ost << "Orbits on points:\\\\" << endl;
	Sch1->Forest->print_orbit_lengths_tex(ost);

	ost << "Orbits on lines:\\\\" << endl;
	Sch2->Forest->print_orbit_lengths_tex(ost);

	ost << "Fixed points:\\\\" << endl;
	Sch1->Forest->print_fixed_points_tex(ost);

	ost << "Fixed lines:\\\\" << endl;
	Sch2->Forest->print_fixed_points_tex(ost);
	...
	if (PA->f_has_action_on_planes) {
		...
		Sch3->Forest->print_fixed_points_tex(ost);
		...
		int verbose_level)
{
	...
	Sch1->Forest->get_orbit_partition(S1, 0 /*verbose_level*/);
	if (f_v) {
		cout << "top_level_geometry_global::report_decomposition_by_single_automorphism "
				"before Sch2->get_orbit_partition" << endl;
	}
	Sch2->Forest->get_orbit_partition(S2, 0 /*verbose_level*/);
	if (f_v) {
		cout << "top_level_geometry_global::report_decomposition_by_single_automorphism "
