	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		...
		else if (ST.stringcmp(argv[i], "-Pentahedral_form") == 0) {
			f_Pentahedral_form = true;
			Pentahedral_form_params25_text.assign(argv[++i]);
			if (f_v) {
				cout << "-Pentahedral_form "
						<< Pentahedral_form_params25_text
						<< endl;
			}
		}
