		...
		std::ostream &ost, int verbose_level)
{
	...
	ost << "\\subsection*{Orbit on single sixes " << orbit_idx << " / "
		<< SOA->SOO->Orbits_on_single_sixes->Forest->nb_orbits << "}" << endl;
		...
		int orbit_idx, int verbose_level)
{
	...
	f = SOA->SOO->Orbits_on_single_sixes->Forest->orbit_first[orbit_idx];
	l = SOA->SOO->Orbits_on_single_sixes->Forest->orbit_len[orbit_idx];
	if (f_v) {
		cout << "orbit " << orbit_idx << " has f=" << f <<  " l=" << l << endl;
	}
	hds = SOA->SOO->Orbits_on_single_sixes->Forest->orbit[f];
	...
	if (f_v) {
		...
		Lint_vec_print(cout, Clebsch_map->Arc, 6);
		cout << " : blown up lines = ";
		Lint_vec_print(cout, Clebsch_map->Blown_up_lines, 6);
		cout << endl;
	}
