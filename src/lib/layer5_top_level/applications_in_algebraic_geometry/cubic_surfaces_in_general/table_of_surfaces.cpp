		...
		int verbose_level)
{
	...
	for (h = 0; h < nb_cubic_surfaces; h++) {
		...
		Table[h * nb_cols + 11] = std::to_string(SOG[h].SOO->Orbits_on_Eckardt_points->Forest->nb_orbits);
		Table[h * nb_cols + 12] = std::to_string(SOG[h].SOO->Orbits_on_Double_points->Forest->nb_orbits);
		Table[h * nb_cols + 13] = std::to_string(SOG[h].SOO->Orbits_on_points_not_on_lines->Forest->nb_orbits);
		Table[h * nb_cols + 14] = std::to_string(SOG[h].SOO->Orbits_on_lines->Forest->nb_orbits);
		Table[h * nb_cols + 15] = std::to_string(SOG[h].SOO->Orbits_on_single_sixes->Forest->nb_orbits);
		Table[h * nb_cols + 16] = std::to_string(SOG[h].SOO->Orbits_on_tritangent_planes->Forest->nb_orbits);
		Table[h * nb_cols + 17] = std::to_string(SOG[h].SOO->Orbits_on_Hesse_planes->Forest->nb_orbits);
		Table[h * nb_cols + 18] = std::to_string(SOG[h].SOO->Orbits_on_trihedral_pairs->Forest->nb_orbits);
