...
interface_toolkit::interface_toolkit()
{
	...
	f_draw_factor_poset = false;
	//draw_factor_poset_graph_fname;
	Factor_poset_draw_options = NULL;
		...
		std::string *argv, int i, int verbose_level)
{
	...
	else if (ST.stringcmp(argv[i], "-draw_factor_poset") == 0) {
		cout << "-draw_factor_poset <string : fname> <layered_graph_options>" << endl;
	}
		...
		std::string *argv, int &i, int verbose_level)
{
	...
	// interface_toolkit_3.csv:
		...
		f_draw_factor_poset = true;
		draw_factor_poset_graph_fname.assign(argv[++i]);
		if (f_v) {
			cout << "-draw_factor_poset " << endl;
		}
		Factor_poset_draw_options = NEW_OBJECT(other::graphics::draw_options);
		i += Factor_poset_draw_options->read_arguments(argc - i - 1,
				argv + i + 1, verbose_level);
		if (f_v) {
			cout << "interface_toolkit::read_arguments "
					"finished reading -draw_factor_poset" << endl;
			cout << "i = " << i << endl;
			cout << "argc = " << argc << endl;
			if (i < argc) {
				cout << "next argument is " << argv[i] << endl;
...
void interface_toolkit::print()
{
	...
	if (f_draw_factor_poset) {
		cout << "-draw_factor_poset " << endl;
		Factor_poset_draw_options->print();
	}
	if (f_read_gedcom) {
		cout << "-read_gedcom "
				<< " " << read_gedcom_fname
		...
		int verbose_level)
{
	...
	if (f_csv_file_join) {
		...
		for (h = loop_from; h < loop_to; h += loop_step) {
			...
			layer6_user_interface::control_everything::orbiter_parse_and_execute(
					layer5_applications::user_interface::core_system::The_Orbiter_top_level_session,
		...
		//layer5_applications::user_interface::core_system::The_Orbiter_top_level_session->parse_and_execute(
		//		argc2 - 1, argv2, 0, verbose_level);
		layer6_user_interface::control_everything::orbiter_parse_and_execute(
				layer5_applications::user_interface::core_system::The_Orbiter_top_level_session,
				argc2 - 1, argv2, 0, verbose_level);
		...
		int verbose_level)
{
	...
	if (f_plot_function) {
	...
	else if (f_draw_factor_poset) {

		if (f_v) {
			cout << "interface_toolkit::worker "
					"-draw_factor_poset "
					<< " " << draw_factor_poset_graph_fname
					<< " " << Factor_poset_draw_options
					<< endl;
		}
		...
		GO.draw_factor_poset_from_file(
				draw_factor_poset_graph_fname,
				Factor_poset_draw_options,
				verbose_level);
		...
		M = Fio.Csv_file_support->read_lint_matrix_csv(
				eliminate_duplicate_columns_fname,
				verbose_level);
		...
		// tally_vector_data is only available for int, not long int
		...
		other::data_structures::lint_matrix *M2;
		other::data_structures::lint_matrix *M3;
		other::data_structures::lint_matrix *M4;

		M2 = NEW_OBJECT(other::data_structures::lint_matrix);
		M2->allocate(Tally.nb_types, Tally.data_set_sz);
		//M2 = NEW_lint(Tally.nb_types * Tally.data_set_sz);
		for (i = 0; i < Tally.nb_types; i++) {
			Int_vec_copy_to_lint(Reps_sorted + i * Tally.data_set_sz, M2->Data + i * Tally.data_set_sz, Tally.data_set_sz);
		}

		M3 = M2->transpose();


		M4 = NEW_OBJECT(other::data_structures::lint_matrix);
		M4->allocate(Tally.nb_types, 1);

		for (i = 0; i < Tally.nb_types; i++) {
			M4->s_ij(i, 0) = Frequency_in_lex_order[i];
		}
		...
		fname_out = eliminate_duplicate_columns_fname;

		String.chop_off_extension(fname_out);

		fname_out += "_reduced_transposed.csv";

		M2->write_csv(fname_out, verbose_level);
		...
		fname_out += "_reduced.csv";

		M3->write_csv(fname_out, verbose_level);
		...
		fname_out += "_frequency.csv";

		M4->write_csv(fname_out, verbose_level);
