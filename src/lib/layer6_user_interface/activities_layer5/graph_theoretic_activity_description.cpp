	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		if (ST.stringcmp(argv[i], "-find_cliques") == 0) {
			...
			if (f_v) {
				cout << "-test_automorphism_property_of_group "
						<< test_automorphism_property_of_group_label << endl;
			}
		}
			...
			if (f_v) {
				cout << "-split_by_starters " << split_by_starters_fname_reps
						<< " " << split_by_starters_col_label << endl;
			}
		}
			...
			if (f_v) {
				cout << "-split_by_clique " << split_by_clique_label
						<< " " << split_by_clique_set << endl;
			}
		}
