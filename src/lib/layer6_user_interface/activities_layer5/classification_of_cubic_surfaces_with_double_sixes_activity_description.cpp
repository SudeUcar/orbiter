	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		...
		if (ST.stringcmp(argv[i], "-report") == 0) {
			...
			sweep_Sylvester_col_ago_label.assign(argv[++i]);
			sweep_Sylvester_options.assign(argv[++i]);
			if (f_v) {
				cout << "-sweep_Sylvester " << sweep_Sylvester_fname
						<< " " << sweep_Sylvester_col_label
						<< " " << sweep_Sylvester_col_ago_label
						<< " " << sweep_Sylvester_options << endl;
			}
		}
			...
			Sylvester_pentahedral_form_after_sweep_col_ago_label.assign(argv[++i]);
			if (f_v) {
				cout << "-Sylvester_pentahedral_form_after_sweep "
						...
						<< " " << Sylvester_pentahedral_form_after_sweep_col_ago_label
						<< endl;
			}
		}
