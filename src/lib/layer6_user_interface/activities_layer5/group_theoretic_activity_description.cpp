...
group_theoretic_activity_description::group_theoretic_activity_description()
{
	...
	f_create_flag_transitive_geometry = false;
	...
	int verbose_level)
{
	...
	for (i = 0; i < argc; i++) {
		...
		if (ST.stringcmp(argv[i], "-report") == 0) {
			...
			element_processing_descr = NEW_OBJECT(layer5_applications::apps_algebra::element_processing_description);
			if (f_v) {
		...
		else if (ST.stringcmp(argv[i], "-diagram") == 0) {
			f_diagram = true;
			if (f_v) {
				cout << "-diagram " << endl;
			}
		...
		else if (ST.stringcmp(argv[i], "-create_flag_transitive_geometry") == 0) {
			f_create_flag_transitive_geometry = true;
			create_flag_transitive_geometry_P.assign(argv[++i]);
			create_flag_transitive_geometry_Q.assign(argv[++i]);
			if (f_v) {
				cout << "-create_flag_transitive_geometry"
						<< " " << create_flag_transitive_geometry_P
						<< " " << create_flag_transitive_geometry_Q
						<< endl;
			}
		}
		...
		// ovoids:
			...
			Ovoid_classify_description = NEW_OBJECT(layer5_applications::apps_geometry::ovoid_classify_description);
			if (f_v) {
...
void group_theoretic_activity_description::print()
{
	...
	if (f_create_flag_transitive_geometry) {
		cout << "-create_flag_transitive_geometry"
				<< " " << create_flag_transitive_geometry_P
				<< " " << create_flag_transitive_geometry_Q
				<< endl;
